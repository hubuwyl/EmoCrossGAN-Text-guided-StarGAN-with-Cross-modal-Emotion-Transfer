...
class Solver(object):
    ...
    def classification_loss(self, logit, target, dataset='CelebA'):
        ...
        # 定义计算 FID 的函数
    def calculate_fid(self,real_images, fake_images, device):
        ...
        real_features = []
        with torch.no_grad():
            for i in range(0, len(real_images), 32):
                batch = real_images[i:i + 32].to(device)
                # 将 100x100 的图像调整为 299x299
                batch = F.interpolate(batch, size=(299, 299), mode='bilinear', align_corners=False)
                features = inception_model(batch)
                if isinstance(features, tuple):
                    features = features[0]  # 处理可能的元组输出
        ...
        fid = diff.dot(diff) + np.trace(real_cov + fake_cov - 2 * covmean)
        return fid
    ...
    def train(self):
        ...
        for i in range(start_iters, self.num_iters):
            ...
            #迭代计算并打印 FID
            if (i + 1) % 10 == 0:
                fid = self.calculate_fid(x_real, x_fake, self.device)
                print(f'Iteration {i + 1}, FID: {fid}')
